...
more_value_features = []  # 存放变量取值大于5
less_value_features = []  # 存放变量取值少于5
# 第一步，检查类别型变量中，哪些变量取值超过5
for var in cat_features:
    valueCounts = len(set(trainData[var]))
    print '{}变量的取值个数为{}'.format(var, valueCounts)
    if valueCounts > 5:
...
for col in num_features:
    ...
    if -1 not in set(trainData[col]):   # －1会当成特殊值处理。如果没有－1，则所有取值都参与分箱
        ...
        cutOff = sf.ChiMerge(trainData, col, 'y', max_interval=max_interval, special_attribute=[-1], minBinPcnt=0)
